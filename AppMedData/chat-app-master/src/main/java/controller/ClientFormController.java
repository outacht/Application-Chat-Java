...
//              #0693e3 #37d67a #40bf75
                ...
                // Ajouter l'heure actuelle au message
                String stringTime = LocalTime.now().format(DateTimeFormatter.ofPattern("HH:mm"));
                Text time = new Text(clientName + " - " + stringTime);
    ...
    public static void receiveMessage(String msg, VBox vBox) throws IOException {
        if (msg.matches(".*\\.(png|jpe)$")){
            ...
            HBox hBox = new HBox();
            HBox hBoxTime = new HBox();//moi==============
            hBox.setAlignment(Pos.CENTER_LEFT);
            hBox.setPadding(new Insets(5,5,5,10));

            HBox hBoxName = new HBox();
            hBoxName.setAlignment(Pos.CENTER_LEFT);
            hBoxTime.setPadding(new Insets(0, 5, 5, 10));//pour moi ========
            String stringTime = LocalTime.now().format(DateTimeFormatter.ofPattern("HH:mm"));//pour moi aussi
